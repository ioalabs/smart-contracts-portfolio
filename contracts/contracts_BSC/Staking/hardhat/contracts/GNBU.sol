...
contract GNBU is Ownable, Pausable {
    ...
    uint256 public supportUnitsCnt;
    ...
    function freeCirculation() external view returns (uint256) {
        ...
        for (uint256 i; i < supportUnits.length; i++) {
            systemAmount = add96(
                systemAmount,
                _unfrozenBalances[supportUnits[i]],
                "GNBU::freeCirculation: adding overflow"
            );
        }
        return
            sub96(
                totalSupply,
                systemAmount,
                "GNBU::freeCirculation: amount exceed totalSupply"
            );
    }
        ...
        returns (bool)
    {
        require(
            spender != address(0),
            "GNBU::approve: approve to the zero address"
        );
    ...
    function balanceOf(address account) public view returns (uint256) {
        ...
        for (uint32 i = 1; i <= _vestingNonces[account]; i++) {
            uint96 unvested = sub96(
                _vestingAmounts[account][i],
                _unvestedAmounts[account][i],
                "GNBU::balanceOf: unvested exceed vested amount"
            );
    ...
    function vestingInfo(address user, uint32 nonce)
        external
        view
        returns (
        ...
        returns (bool)
    {
        uint96 amount = safe96(
            rawAmount,
            "GNBU::transfer: amount exceeds 96 bits"
        );
    ...
    function transferFrom(
        ...
        uint96 amount = safe96(
            rawAmount,
            "GNBU::approve: amount exceeds 96 bits"
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "GNBU::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "GNBU::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "GNBU::delegateBySig: signature expired"
        );
    ...
    function unvest() external whenNotPaused returns (uint256 unvested) {
        ...
        for (uint32 i = 1; i <= _vestingNonces[msg.sender]; i++) {
            ...
            require(
                ...
                "GNBU::unvest: already unvested amount exceeds toUnvest"
            );
            toUnvest -= _unvestedAmounts[msg.sender][i];
            unvested += toUnvest;
            _unvestedAmounts[msg.sender][i] = safe96(
                totalUnvestedForNonce,
                "GNBU::unvest: amount exceeds 96 bits"
            );
        }
        _unfrozenBalances[msg.sender] = add96(
            _unfrozenBalances[msg.sender],
            safe96(unvested, "GNBU::unvest: amount exceeds 96 bits"),
            "GNBU::unvest: adding overflow"
        );
    ...
    function _transferTokens(
        ...
        _unfrozenBalances[src] = sub96(
            _unfrozenBalances[src],
            amount,
            "GNBU::_transferTokens: transfer amount exceeds balance"
        );
        _unfrozenBalances[dst] = add96(
            _unfrozenBalances[dst],
            amount,
            "GNBU::_transferTokens: transfer amount overflows"
        );
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(
                    srcRepOld,
                    amount,
                    "GNBU::_moveVotes: vote amount underflows"
                );
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(
                    dstRepOld,
                    amount,
                    "GNBU::_moveVotes: vote amount overflows"
                );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "GNBU::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    function _vest(address user, uint96 amount) private {
        ...
        _vestingReleaseStartDates[user][nonce] =
            block.timestamp +
            vestingFirstPeriod;
        _unfrozenBalances[owner] = sub96(
            _unfrozenBalances[owner],
            amount,
    ...
    function burnTokens(uint256 rawAmount)
        public
        onlyOwner
        returns (bool success)
    {
        uint96 amount = safe96(
            rawAmount,
            "GNBU::burnTokens: amount exceeds 96 bits"
        );
        require(amount <= _unfrozenBalances[owner]);
        _unfrozenBalances[owner] = sub96(
            _unfrozenBalances[owner],
            amount,
            "GNBU::burnTokens: transfer amount exceeds balance"
        );
        totalSupply = sub96(
            totalSupply,
            amount,
            "GNBU::burnTokens: transfer amount exceeds total supply"
        );
        ...
        returns (uint256)
    {
        ...
        for (uint256 i; i < to.length; i++) {
            _unfrozenBalances[to[i]] = add96(
                _unfrozenBalances[to[i]],
                uint96(values[i]),
                "GNBU::_transferTokens: transfer amount exceeds balance"
            );
    ...
    function updateSupportUnitRemove(uint256 supportUnitIndex)
        external
        onlyOwner
    {
