...
contract TakeIT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable {
    ...
    event ConfirmOrder(uint256 indexed tokenId, uint256 nbuToSwap, uint256 resultedBusd, bool isMannual);
    ...
    function getListAmountsRates(uint256 from, uint256 to) public view returns (Rates[] memory){
       uint256 length = to - from;
       Rates[] memory RatesList = new Rates[](length);
       for (uint256 i = 0; i < length; i++) {
           Rates memory rates;
           uint256 tokenId = from + i;
           (rates.NbuBnbRate, rates.NbuAmount, rates.BusdBnbRate, rates.NbuBusdRate) = getAmountsAndRates(tokenId);
    ...
    function burnNFT(uint256 tokenId) external {
     ...
     function getBusdBNBamount(uint256 tokenId,uint256 amountNbu) public view returns(uint256 amountBusd, uint amountBNB,uint BusdNbuRate,uint BnbNburate,uint NBUAmount,uint BnbBusdRate) {
        ...
        (uint[] memory amountsBNBBusd) = IPancakeRouter(pancakeRouter).getAmountsOut(
            amountBNB, 
            path
        );
    ...
    function supplyBusd(uint256 tokenId, uint256 busdToSend) external onlyAllowedUpdaters {
        ...
        require(reserveBusd >= busdToSend, "BUSD token reserve is insufficient");
    ...
    function updateTokens(address newPurchaseToken, address newBUSD) external onlyOwner {
        require(AddressUpgradeable.isContract(newPurchaseToken), "NFT: purchase token is not a contract");
        require(AddressUpgradeable.isContract(newBUSD), "NFT: busd token is not a contract");
